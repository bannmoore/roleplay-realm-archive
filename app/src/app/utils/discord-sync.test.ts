    ...
    test("syncs message attachments through channel sync", async () => {
      ...
      when(storage.uploadFile)
        .calledWith({
          ...
          path: `message-attachments/channel-${createdMessage.channelId}/message-${createdMessage.id}/${discordMessage.attachments[0].id}-${discordMessage.attachments[0].filename}`,
        })
        .thenResolve("/uploaded-path-1/file.png");
      when(storage.uploadFile)
        .calledWith({
          buf: expect.any(ArrayBuffer),
          path: `message-attachments/channel-${createdMessage.channelId}/message-${createdMessage.id}/${discordMessage.attachments[1].id}-${discordMessage.attachments[1].filename}`,
        })
